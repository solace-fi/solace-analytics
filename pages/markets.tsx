...
const SwapSimulator: any = (props: any) => {
  ...
  return (<>
    <p>Note: negative amount means tokens transfer from pool to you, positive amount means tokens transfer from you to pool</p>

    <h4>Mainnet Sushiswap</h4>
    <p>{`This pool has ${Math.floor(mainnetReserveSolace).toLocaleString()} SOLACE and ${Math.floor(mainnetReserveUsd).toLocaleString()} USD`}</p>
    <p>Removing/adding X USD will cost/give me Y SOLACE</p>
    <SolaceUsdSwapAmountSimulatorChart solaceByUsd={mainnetSims.solaceByUsd}/>
    <p>Removing/adding X USD will change the SOLACE/USD price to Y</p>
    <SolaceUsdSwapPriceUsdSimulatorChart priceByUsd={mainnetSims.priceByUsd} price={mainnetSims.price}/>
    <p>Removing/adding X SOLACE will change the SOLACE/USD price to Y</p>
    <SolaceUsdSwapPriceSolaceSimulatorChart priceBySolace={mainnetSims.priceBySolace} price={mainnetSims.price}/>

    <h4>Aurora Trisolaris</h4>
    <p>{`This pool has ${Math.floor(mainnetReserveSolace).toLocaleString()} SOLACE and ${Math.floor(mainnetReserveUsd).toLocaleString()} USD of NEAR`}</p>
    <p>Removing/adding X USD will cost/give me Y SOLACE</p>
    <SolaceUsdSwapAmountSimulatorChart solaceByUsd={auroraSims.solaceByUsd}/>
    <p>Removing/adding X USD will change the SOLACE/USD price to Y</p>
    <SolaceUsdSwapPriceUsdSimulatorChart priceByUsd={auroraSims.priceByUsd} price={auroraSims.price}/>
    <p>Removing/adding X SOLACE will change the SOLACE/USD price to Y</p>
    <SolaceUsdSwapPriceSolaceSimulatorChart priceBySolace={auroraSims.priceBySolace} price={auroraSims.price}/>
  </>)
}
